...
- `src/viewer/RoomBlock.tsx` — a parametric room-shell primitive: floor slab, opaque ceiling slab, and four 6"-thick walls (+ wireframe edges), sized in meters via `position`/`size` props, with `floorMaterial`/`wallMaterial` props (see `src/materials.ts`). `size` is the room's outer footprint; walls are centered on that boundary (`WALL_THICKNESS`, exported from this file). The floor uses a procedural tile texture (`src/textures.ts`) when the material's `pattern` is `'tile'`. This is the seed for what will become the library of architectural primitives — new primitives should follow the same pattern: a typed props object describing real-world dimensions, rendered as plain Three.js meshes.
