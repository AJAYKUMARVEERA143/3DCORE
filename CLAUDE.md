...
- `src/viewer/RoomBlock.tsx` — a parametric room-shell primitive (floor slab + translucent wall box + wireframe edges), sized in meters via `position`/`size` props, with `floorMaterial`/`wallMaterial` props (see `src/materials.ts`). This is the seed for what will become the library of architectural primitives (walls, rooms, furniture, etc.) — new primitives should follow the same pattern: a typed props object describing real-world dimensions, rendered as plain Three.js meshes.
- `src/viewer/BedBlock.tsx` — first furniture primitive (frame, mattress, pillow, headboard), same parametric-primitive pattern. Its footprint constant lives in `src/furniture.ts` (not the component file) so it can be imported by `Viewer` for placement without breaking Fast Refresh.
